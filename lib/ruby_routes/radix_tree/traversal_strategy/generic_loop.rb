      ...
      class GenericLoop < Base
        def execute(segments, state, method, params, captured_params)
          segments.each_with_index do |segment, index|
            next_node, stop = @finder.traverse_for_segment(state[:current], segment, index, segments, params, captured_params)
            return @finder.finalize_on_fail(state, method, params, captured_params) unless next_node
