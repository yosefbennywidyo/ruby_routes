...
RSpec.describe RubyRoutes::Route do
  describe '#initialize' do
    ...
    it 'accepts custom HTTP methods' do
      route = RubyRoutes::Route.new('/users', via: :post, to: 'users#create')
      expect(route.methods).to eq(['POST'])

      route = RubyRoutes::Route.new('/users', via: [:get, :post], to: 'users#handle')
    ...
    it 'accepts separate controller and action' do
      route = RubyRoutes::Route.new('/users', controller: 'users', action: 'index')
      expect(route.controller).to eq('users')
      expect(route.action).to eq('index')
    end

    it 'raises error without controller' do
      expect { RubyRoutes::Route.new('/users', action: 'index') }.to raise_error(RubyRoutes::InvalidRoute)
    end

    it 'raises error without action' do
      expect { RubyRoutes::Route.new('/users', controller: 'users') }.to raise_error(RubyRoutes::InvalidRoute)
    end
  end
  ...
  describe '#match?' do
    ...
    it 'matches with multiple HTTP methods' do
      route = RubyRoutes::Route.new('/users/:id', via: [:get, :put], to: 'users#show')
      expect(route.match?('GET', '/users/123')).to be true
  ...
  describe '#extract_params' do
    let(:route) { RubyRoutes::Route.new('/users/:id/posts/:post_id', to: 'posts#show') }
    ...
    it 'includes defaults' do
      route = RubyRoutes::Route.new('/users/:id', to: 'users#show', defaults: { format: 'html' })
      params = route.extract_params('/users/123')
